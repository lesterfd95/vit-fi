...
class FaultInjection:
                    ...
                    for i in layer_types:
                        if isinstance(layer, i):
                            ...
                            if hasattr(layer, "weight"):
                                ...
                                weights_shape.append(torch.Size([])) 
            # unpack node
            else:
    ...
    def declare_neuron_fault_injection(self, **kwargs):
        ...
        if kwargs:
            ...
            #print("changing bitflip positions")
        ...
        # Nuevo: si ya existe un corrupted_model, no lo recreamos 
        # Gracias a esto puedo cambiar durante cada batch de entrenamiento las posiciones aleatorias a inyectar
        # llamando a random_neuron_multiple_bit_inj sin que el modelo se reinicie al original  
        # de lo contrario perderia los pesos y tendria que recargarlos, copiar el modelo a la memoria, fijar optimizador y estado train()
        if not hasattr(self, "corrupted_model") or self.corrupted_model is None:
            #print("deepcopy and hooking")
            self.corrupted_model = copy.deepcopy(self.original_model)
